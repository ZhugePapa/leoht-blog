...
3. **浪漫只能藏在一两处**。Source Serif 4 italic 的可斜体、深酒红的极小面积出现、章节首页的中央对称 dropcap——浪漫只在关键时刻出现一两次，其它地方文人克制。整页浪漫 = 文艺青年博客，不是文人杂志。
...
| 角色 | 字体 | 字重 / 处理 | 用法 |
| --- | --- | --- | --- |
| Display | **Source Serif 4** | 400 + 500 | Hero 主标、文章 H1、章节首页大标。与 body 同族衬线，靠 500 字重与字号提级，不靠类别对比。 |
| Body | **Source Serif 4** | 400 + 600 italic | 全文正文。与 display 同族，靠 400 字重 + 正文字号区隔。 |
| Meta | **Noto Sans SC** | 400 / 500，全小写 | 元数据、章节编号、状态印迹、日期、目录编号、标签。**不再等宽**——机构感来自小写 + 字间距 + 灰阶。 |
| CJK 配对 | display + body 配 **思源宋体 Noto Serif SC**，meta 配 **思源黑体 Noto Sans SC** | 400 / 500 | 中文 fallback 与对应西文同衬线/无衬线类别。 |

**不做的事**：
- 不用 Inter、Geist、SF 作为任何角色字体——它们是 SaaS 默认，是气质反方向。
- **无衬线只允许出现在 meta**：Source Serif 4 是 display 与 body 的唯一 face，Noto Sans SC 只做 meta，不进标题或正文。
- 不用 Playfair Display——太营销页、太"婚礼请柬"。
- display 与 body 同族：标题用 500、正文用 400，靠字重 + 字号区隔，不靠换字体。
...
| Meta | 1.4 | `+0.04em` | 400 | 小写 + 字间距 = 机构感（不再等宽） |
| Meta S (印迹) | 1.2 | `+0.06em` | 400 | 印迹字间距最宽 |

中文部分行高略增 0.05（思源宋体比 Source Serif 4 字身大）。

### 2.4 字符处理

- **所有 meta 字段统一小写**：`domain · topic / type / 2026.06.12`。绝不 ALL CAPS——大规模大写 = 营销页"科技感"，我们要"档案馆系统感"（机构感来自小写 + 字间距 + 灰阶，不再依赖等宽）。
- **章节号用罗马数字**：`ⅰ / ⅱ / ⅲ` 而不是 `1 / 2 / 3`。这是文人气质的关键——阿拉伯数字立刻塌成工作手册。
- **Dropcap**：每篇文章首段首字做 dropcap，Source Serif 4 500、4 行高、`vintage-red` 色、左侧 0.5em 内缩。一篇文章只做一处。这也是 vintage-red 最高频的合法使用点。
- **斜体使用规则**：Source Serif 4 italic 用在三种地方：(a) 内文引文（"《思考，快与慢》卡尼曼说……"），(b) 章节副标题，(c) Hero 主标 / 副标中需要强调的一个词。display 与 body 同族，italic 在标题与正文里都可用。

### 2.5 字体加载

- Source Serif 4（display + body 共用）与 Noto Sans SC（meta）必须预加载，避免 FOUT 破坏首屏气质。
- 用 `font-display: swap`，但 meta 字体 Noto Sans SC 用 `font-display: fallback`，因为它出现位置多，swap 闪动会很明显。
- 中文思源宋体/黑体按需子集化，或用 Fontsource 多语言 fallback。
...
```
container-text   680px    正文阅读（杂志型窄，给 Source Serif 4 呼吸）
container        1440px   默认页面内容宽度
container-wide   1600px   图谱、大表格
```

正文 680px 比 v0.1 的 720 还窄——文人杂志气质要 Source Serif 4 在一行里只跑大约 12-14 个西文词，超过就散气。页面内容宽度 1440px 是全站主容器（`.page`），宽容器 1600px 留给图谱与大表格。

### 4.2 通用页眉页脚（"页码系统"）

每个页面顶部一条 `rule-strong` 横线，下方一行 meta 页眉：
...
- 左：站点名（小写 meta）+ emdash + `revue`（用"杂志"的法文词比 magazine高级）
...
- **章节首页**（专题集子）：中央对称布局，Source Serif 4 大标居中、薄双线、dropcap——是文人杂志里少数允许中央对齐的页面。
...
```
┌────────────────────────────────────────────────┐
│ № ⅴ · psychological effect                    │ ← meta，罗马编号 + 内容类型
├────────────────────────────────────────────────┤
│                                                │
│ 损失厌恶                                       │ ← Source Serif 4 500 22px
│ 为什么"失去"比"得到"更刺激                       │ ← Source Serif 4 italic 18px 副标
│                                                │
│ 人对同等损失的痛苦感受约是同等收益的两倍……       │ ← Source Serif 4 18px
│                                                │
├────────────────────────────────────────────────┤
│ domain · behavior    topic · 行为经济学          │ ← meta 13px
│ tags · 决策 / 订阅转化 / 增长                   │
└────────────────────────────────────────────────┘
```
...
见 1.3。单色印迹、字母、1px 内嵌压痕。印迹本身用 Source Serif 4 400 小写而不是 meta（Noto Sans SC）——serif 小写更"印章"感，与正文同族一体。
...
```
[vellum] · 1px rule · 4px radius · meta 13px
...
- 罗马编号 meta、标题 Source Serif 4 500 22px、上下细线 `rule`、页码 meta。
...
- 主操作（如"进入内容库"）：`No ⅴ · Enter the library →`，meta 小写 + Source Serif 4 italic 主词 + `→` 前缀。
- 次操作：纯文字 + 薄下划线。
...
```
[letter: Source Serif 4 500, 4-line height, vintage-red 色, line-height 0.85, margin-right 0.08em, margin-top 0.04em]
```
...
> 注：拉引文不使用大引号符号 " "——靠 Source Serif 4 italic 与左侧 vintage-red 竖线足以构成视觉引文。加引号符号立刻塌成博客。

- Source Serif 4 italic 400 1.5rem、行高 1.55
...
- 节点用 Source Serif 4 500 的单字母 + 1px 描边方块，**不是圆球**。
- 连线用 `rule` 直线，当前文章节点描边变 `muted-gold`、其他 hover 节点变 `ink-blue`。
- 节点 hover 显示 meta 工具提示，不是浮卡。
...
┌─────────────────────┐
│                     │
│   № ⅴ · BEH · 04     │ ← Source Serif 4 500 编号
│   loss-aversion      │ ← meta slug 作为印迹文
│   revue · 2026 · vi │ ← meta 月度卷号
│                     │
└─────────────────────┘
```

- 1px 内嵌压痕边线（`inset 1px 1px 0 var(--charcoal / 20%)`），内部 `vellum`
- 全部文字 charcoal，编号 Source Serif 4 500，slug 与卷号 meta
...
═══════════════════════════════════════════════════════════════════
   ...
   revue for the                                                  ← Source Serif 4 400 67px
...
───────────────────────────────────────────────────────────────────
  ...
  in this issue                                       ← Source Serif 4 italic 38px H2
...
═══════════════════════════════════════════════════════════════════
  ...
  [italic Source Serif 4] 为什么"失去"比"得到"更刺激    │                  │
                                                └─────────────────┘
───────────────────────────────────────────────────────────────────
...
───────────────────────────────────────────────────────────────────
   ...
   F     损失厌恶不是用来"骗"用户的——它是对                ← Dropcap Source Serif 4 500
   ┌──  着用户 Already 已有心理预期的一种设计                │ vintage-red 色，4 行高
         语言。卡尼曼在《思考，快与慢》中提出：……
                                                                          [内文链接 ink-blue → hover vintage-red]

  Ⅰ. 免费试用的"拥有感"陷阱                                       ← Source Serif 4 500 罗马数字章节号
  ──────────────                                                  ← rule-strong 章节下划线
...
在 Astro 项目初始化时定下来：CSS variables、Tailwind config、Source Serif 4（display + body）+ Noto Sans SC（meta）两族（Fontsource 预载）、状态印迹组件、TagChip、Dropcap、Pullquote 三个排版本地组件。

**字体加载是 Phase 1 的硬指标**——FOUT 破坏巴黎文人气质最严重，必须 `preload` + `font-display: swap` 调好。

### Phase 2：Home 与 Library

替换现有 React 组件为杂志型布局。Library 列表式，Home Hero 用 Source Serif 4 大标分两行。
...
Collection 章节首页中央对称版式在此阶段做。LocalGraph 节点 Source Serif 4 单字母方块。最后做一次"减一件配饰"检查（见 SKILL.md 的 Chanel 原则）——尤其要复核印迹旋转角度、dropcap 出现次数、vintage-red 总面积比例（≤ 3%）。
...
8. **大引号符号 " "**：pullquote 不用引号符号——用 Source Serif 4 italic 与 vintage-red 竖线足以。加大引号立刻塌。
9. **模仿真实杂志纸边毛糙**：完全不做。
10. **混入无衬线正文/标题**：display 与 body 都是 Source Serif 4 衬线，meta 是 Noto Sans SC 无衬线。无衬线只允许在 meta，进了标题或正文就立刻塌成 SaaS。
11. **meta 字体进正文/标题**：Noto Sans SC 只做 meta（页眉、标签、印迹、编号），不能进标题或正文。标题与正文 100% 由 Source Serif 4 渲染。
...
- ~~display 选型~~：**已定 — Source Serif 4（与 body 同族衬线）**。变更史：v0.2 初版选 Cormorant Garamond（Paris Review 文人高对比衬线）；2026-07-03 一次改向 IBM Plex Sans 无衬线（"标题无衬线 vs 正文衬线"类别对比）；2026-07-03 二次改回 Source Serif 4 与 body 同族——标题与正文靠 500 vs 400 字重 + 字号区隔，不靠换字体。原 Cormorant vs EB Garamond vs Bodoni Moda 衬线对比段作废。
- 中文配对：**已定 — display + body 配思源宋体（Noto Serif SC）400/500/700，meta 配思源黑体（Noto Sans SC）400/500**。中文 fallback 与对应西文同衬线/无衬线类别。衡山毛筆フォント不做正文。
- 卷期号的实际使用方式：是按月份（vol ⅲ = 6 月）还是按季度？MVP 阶段每月一卷是否太密？MVP 阶段先不引入卷期号、Phase 2 再加，可能更稳。
- 印迹方块是否真的需要"压痕 inset shadow"，还是 1px 内嵌线足够——必须看到真实渲染才知道。过度模拟纸感会落入"做旧"陷阱。**当前实现用 `inset 1px 1px 0 var(--rule)` 内嵌线，未用更强压痕；hero-stamp 已按 §3.4 改为 inset，删除了原来的外阴影。**
- **DESIGN.md 隐含的 Bodoni Moda 选型 vs 本版选型**：DESIGN.md typography 段写的是 Bodoni Moda（高对比衬线），本版最终选 Source Serif 4（与 body 同族衬线）。变更史见上一条。
- **ink-blue 的中文 fallback**：`#0D1B2A` 中文配对应选思源宋体 Heavy 还是别的？色相协调要在渲染后定。**已定：ink-blue 出现在链接与 meta，统一用思源宋体 400，不引入 Heavy。**
...
| 正文字体 | Source Serif 4（同主体） | Source Serif 4 主标 + Source Serif 4 正文 + Noto Sans SC meta |
...
V0.2 在结构性骨架上继承 V0.1（编号、印迹、知识卡片直角描边、链接无按钮），色彩系统改用 DESIGN.md（Atelier du Savoir）六色色板；本版本对色板未规定的 ink-blue 赋予"内文链接常驻色"角色，对状态印迹的彩色信号从 vintage-red 改为 muted-gold 以守住"vintage-red ≤ 3%"配额。**2026-07-03 字体改向（二次）：display 由 IBM Plex Sans 无衬线改回 Source Serif 4，与 body 同族衬线——标题与正文靠 500 vs 400 字重 + 字号区隔，回归"衬线一以贯之"。meta 仍 Noto Sans SC（不再等宽）。变更史：Cormorant Garamond → IBM Plex Sans → Source Serif 4。**